  ...
  for (const col of data.collections) {
    if (col.subCollections) {
      for (const sub of col.subCollections) {
        for (const p of sub.products ?? []) result.push({ ...p, collectionLabel: col.title, subCollectionLabel: sub.title });
      }
    } else {
      for (const p of col.products ?? []) result.push({ ...p, collectionLabel: col.title, subCollectionLabel: null });
    }
  }
  return result;
}
...
export default function WomensProductDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const allProducts = flattenAllProducts(arr[0]);
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-white text-stone-800" style={{ fontFamily: "'Cairo', sans-serif" }}>
      ...
      {/* Breadcrumb */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-3 sm:py-4 text-xs text-stone-400 tracking-widest flex items-center gap-1.5 sm:gap-2 flex-wrap">
        <Link href="/" className="hover:text-stone-700 transition-colors">الرئيسية</Link>
        <span>/</span>
        <Link href="/Womens" className="hover:text-stone-700 transition-colors">تسوق الحريم</Link>
        {product.collectionLabel && (<><span>/</span><span className="text-stone-500 hidden sm:inline">{product.collectionLabel}</span></>)}
        {product.subCollectionLabel && (<><span>/</span><span className="text-stone-500 hidden sm:inline">{product.subCollectionLabel}</span></>)}
        <span>/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-12 lg:gap-20">
          ...
          <div className="animate-fade-in">
            <div className="bg-[#f0efeb] aspect-[3.5/4] overflow-hidden relative">
              ...
              <img src={product.image} alt={product.name} className="w-full h-full object-cover" onError={(e) => { e.target.src = "https://images.unsplash.com/photo-1522335789203-aabd1fc54bc9?w=400&q=80"; }} />
            </div>
          </div>
          ...
          <div className="animate-fade-in space-y-4 sm:space-y-6" style={{ animationDelay: "100ms" }}>
            ...
            <div className="space-y-3 pt-1">
              ...
              <button
                ...
                className={`w-full py-3.5 sm:py-4 text-xs sm:text-sm font-bold tracking-widest transition-all duration-200 border ${
                  product.inStock ? adding ? "bg-stone-600 text-white border-stone-600 cursor-wait" : "bg-black text-white border-black hover:bg-white hover:text-black" : "bg-stone-200 text-stone-400 border-transparent cursor-not-allowed"
                }`}
              >
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-8 sm:py-12">
            <div className="max-w-3xl">
              {details.targets?.length > 0 && (
                <DetailRow label="مناسب لـ">
                  <div className="flex flex-wrap gap-2">{details.targets.map((t, i) => <span key={i} className="border border-stone-200 text-stone-600 text-xs px-2.5 py-1 sm:px-3 tracking-wide">{t}</span>)}</div>
                </DetailRow>
              )}
              {details.features?.length > 0 && (
                <DetailRow label="المميزات">
                  <div className="space-y-2">
                    {details.features.map((f, i) => (
                      <div key={i} className="flex items-center gap-2">
                        <svg className="w-4 h-4 text-stone-700 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                        <span>{f}</span>
                      </div>
                    ))}
                  </div>
