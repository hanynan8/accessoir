...
function FilterDropdown({ label, options, value, onChange }) {
  ...
  const toggleParent = (pv) => setExpandedParents(prev => ({ ...prev, [pv]: !prev[pv] }));
          ...
          {hasSubCollections && structured ? (
            structured.map(({ parent, children }) => (
              <div key={parent.value} className="border-b border-stone-100 last:border-b-0">
                {children.length > 0 ? (
                  <>
                    <button onClick={() => toggleParent(parent.value)} className={`w-full text-right px-4 py-3 text-sm font-semibold flex items-center justify-between transition-colors ${expandedParents[parent.value] ? "bg-stone-50 text-stone-800" : "text-stone-700 hover:bg-stone-50"}`}>
...
function MobileFilterSheet({ open, onClose, collections, activeCollection, setActiveCollection, activeDiscount, setActiveDiscount, sortBy, setSortBy, filtered, resetFilters }) {
  ...
  const discountOpts = [{ value: "all", label: "الكل" }, { value: "discount", label: "عروض فقط" }, { value: "instock", label: "متوفر فقط" }];
  const sortOpts = [{ value: "default", label: "المميز" }, { value: "price-asc", label: "الأقل سعراً" }, { value: "price-desc", label: "الأعلى سعراً" }, { value: "rating", label: "الأعلى تقييماً" }, { value: "discount", label: "الأعلى خصماً" }];
          ...
          <div>
            ...
            <div className="space-y-1">
              ...
              <button
                ...
                className={`w-full text-right px-4 py-2.5 rounded-xl text-sm font-semibold border transition-all ${activeCollection === "all" ? "bg-stone-800 text-white border-stone-800" : "border-stone-200 text-stone-600 bg-stone-50"}`}
              >الكل</button>
              ...
              {structured.map(({ parent, children }) => (
                <div key={parent.value}>
                  {children.length > 0 ? (
                    <>
                      {/* Parent row — tap to expand/select */}
                      <button
                        onClick={() => setExpandedParents(p => ({ ...p, [parent.value]: !p[parent.value] }))}
                        className={`w-full text-right px-4 py-2.5 rounded-xl text-sm font-semibold border transition-all flex items-center justify-between ${activeCollection === parent.value ? "bg-stone-800 text-white border-stone-800" : "border-stone-200 text-stone-700 bg-stone-50"}`}
                      >
                        <span>{parent.label}</span>
                      ...
                      {/* Sub-collection pills — indented */}
                      {expandedParents[parent.value] && (
                        <div className="mr-4 mt-1 space-y-1">
                          {children.map(child => (
                            <button
                              key={child.value}
                              onClick={() => setActiveCollection(child.value)}
                              className={`w-full text-right px-4 py-2 rounded-xl text-sm border transition-all flex items-center gap-2 ${activeCollection === child.value ? "bg-stone-700 text-white border-stone-700" : "border-stone-200 text-stone-600 bg-white"}`}
                            >
                    ...
                    <button
                      onClick={() => setActiveCollection(parent.value)}
                      className={`w-full text-right px-4 py-2.5 rounded-xl text-sm font-semibold border transition-all ${activeCollection === parent.value ? "bg-stone-800 text-white border-stone-800" : "border-stone-200 text-stone-700 bg-stone-50"}`}
                    >{parent.label}</button>
                  )}
                </div>
              ))}
...
export default function MensPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch("/api/data?collection=Mens");
        if (!res.ok) throw new Error("فشل تحميل بيانات الرجال");
  ...
  const filtered = useMemo(() => {
    ...
    if (activeCollection !== "all") {
      if (activeCollection.includes("__")) {
        const [parentKey, subKey] = activeCollection.split("__");
        list = list.filter(p => p.collectionKey === parentKey && p.subCollectionKey === subKey);
  ...
  if (error) return (
    <div className="min-h-screen bg-white flex items-center justify-center">
      <div className="text-center space-y-3">
        ...
        <button onClick={() => window.location.reload()} className="border border-stone-300 text-stone-500 px-6 py-2 text-xs tracking-widest hover:border-stone-500 transition-colors">إعادة المحاولة</button>
      </div>
    </div>
  );
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-white text-stone-800" style={{ fontFamily: "'Cairo', sans-serif" }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 pt-7 sm:pt-10 pb-4 sm:pb-6">
        <h1 className="text-2xl sm:text-4xl font-bold text-stone-800 mb-1 sm:mb-2">تسوق الرجال</h1>
        <p className="text-stone-500 text-sm sm:text-base">اكتشف أفضل منتجات الرجال</p>
      </div>
      ...
      <div className="border-t border-b border-stone-200 bg-white sticky top-0 z-30">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-10 py-2.5 sm:py-3 border-b border-stone-100">
          <div className="relative w-full">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="ابحث بالاسم ..." className="w-full border border-stone-200 py-2 sm:py-2.5 pr-9 pl-4 text-sm text-stone-700 placeholder-stone-400 focus:outline-none focus:border-stone-500 transition-colors" />
      ...
      <div className="px-3 sm:px-4 lg:px-[15px] pt-6 sm:pt-10 pb-10">
        {filtered.length === 0 ? (
          <div className="text-center py-24 space-y-3">
            <svg className="w-10 h-10 text-stone-300 mx-auto" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
