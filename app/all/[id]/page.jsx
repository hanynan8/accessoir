// ══════════════════════════════════════════════════
// FILE: app/products/[id]/page.jsx — All product detail
// ══════════════════════════════════════════════════
"use client";
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const isWomens = id.startsWith("ws-") || id.startsWith("wa-") || id.startsWith("wm-");
        const isMens = id.startsWith("ms-") || id.startsWith("ma-");
        let allProducts = [];
        if (isWomens || (!isWomens && !isMens)) {
          const wRes = await fetch("/api/data?collection=Womens");
          if (wRes.ok) { const wArr = await wRes.json(); allProducts = [...allProducts, ...(wArr[0]?.collections ?? []).flatMap((c) => c.products ?? [])]; }
        }
        if (isMens || (!isWomens && !isMens)) {
          const mRes = await fetch("/api/data?collection=Mens");
          if (mRes.ok) { const mArr = await mRes.json(); allProducts = [...allProducts, ...(mArr[0]?.collections ?? []).flatMap((c) => c.products ?? [])]; }
        }
        if (!isWomens && !isMens) {
          const bsRes = await fetch("/api/data?collection=BestSellers");
          if (bsRes.ok) { const bsArr = await bsRes.json(); allProducts = [...allProducts, ...(bsArr[0]?.collections ?? []).flatMap((c) => c.products ?? [])]; }
        }
  ...
  const fallbackImg = id.startsWith("ws-") || id.startsWith("wa-") || id.startsWith("wm-")
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-white text-stone-800" style={{ fontFamily: "'Cairo', sans-serif" }}>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 py-3 sm:py-4 text-xs text-stone-400 tracking-widest flex items-center gap-1.5 flex-wrap">
        ...
        <span>/</span>
        <Link href="/products" className="hover:text-stone-700 transition-colors">جميع المنتجات</Link>
        <span>/</span>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 sm:gap-12 lg:gap-20">
          ...
          <div className="animate-fade-in space-y-4 sm:space-y-6" style={{ animationDelay: "100ms" }}>
            ...
            <div className="flex items-end gap-3 pt-1">
              ...
              {product.discount?.active && product.discount?.originalPrice && <span className="text-stone-400 text-base sm:text-lg line-through mb-0.5">{product.discount.originalPrice} {product.currency}</span>}
            </div>
            {details && (
